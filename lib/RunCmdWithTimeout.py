...
def RunCommand(cmd, timeout_time=None, retry_count=3, return_output=None, stdin_input=None):
  ...
  while True:
    try:
      result = RunOnce(cmd, timeout_time=timeout_time, return_output=return_output, stdin_input=stdin_input)
...
def RunOnce(cmd, timeout_time=None, return_output=None, stdin_input=None):
  ...
  def Run():
    ...
    if stdin_input:
      ...
      stdin_dest = None
    pipe = subprocess.Popen(cmd,stdin=stdin_dest,stdout=output_dest,stderr=subprocess.STDOUT,shell=True)
    #pipe.wait()
    pid.append(pipe.pid)
    try:
  ...
  while not break_loop:
    if not t.isAlive():
      break_loop = True
    # Check the timeout
    if (not break_loop and timeout_time is not None and time.time() > start_time + timeout_time):
      try:
        if len(pid) > 0:
